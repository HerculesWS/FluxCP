...
<br />
<p class="checkout-text"><a href="<?php echo htmlspecialchars($this->url('purchase', 'checkout')) ?>">Proceed to Checkout Area</a></p>
<form action="<?php echo htmlspecialchars($this->url('purchase', 'remove')) ?>" method="post">
	<table class="vertical-table cart">
		...
		<tr>
			...
			<?php if (($item->shop_item_use_existing && ($image=$this->itemImage($item->shop_item_nameid))) || ($image=$this->shopItemImage($item->shop_item_id))): ?>
				<img src="<?php echo $image ?>?nocache=<?php echo rand() ?>" alt="" />
			<?php endif ?>
			</td>
			<td>
				<h4>
...
<form action="<?php echo htmlspecialchars($this->url('purchase', 'clear')) ?>" method="post">
	<p class="remove-from-cart">
